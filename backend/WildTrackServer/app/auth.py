...
def get_auth0_jwks():
    ...
    # Development mode: skip JWKS if Auth0 not configured
    if settings.dev_mode or not settings.auth0_domain:
        logger.warning("⚠️ Development mode: Skipping Auth0 JWKS fetch")
        return {"keys": []}  # Return empty JWKS in dev mode
    ...
    if _jwks_cache is None or _jwks_cache_expiry is None or now > _jwks_cache_expiry:
        ...
        try:
            ...
            if settings.dev_mode:
                logger.warning("⚠️ Could not fetch Auth0 JWKS (dev mode): Timeout")
                return {"keys": []}
            raise HTTPException(
            ...
            # In development, don't fail hard
            if settings.dev_mode:
                logger.warning(f"⚠️ Could not fetch Auth0 JWKS (dev mode): HTTP {e.response.status_code}")
                return {"keys": []}
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            ...
            # In development, don't fail hard
            if settings.dev_mode:
                logger.warning(f"⚠️ Could not fetch Auth0 JWKS (dev mode): {str(e)}")
                return {"keys": []}
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
...
async def get_current_user(
    ...
    In development mode (when Auth0 is not configured), returns a mock user.
    ...
    """
    # Development mode: bypass Auth0 if not configured
    if settings.dev_mode or not settings.auth0_domain or not settings.auth0_api_audience:
        logger.warning("⚠️ Development mode: Using mock user (Auth0 not configured)")
        return {
            "id": "dev-user-123",
            "email": "dev@wildtrack.local",
            "username": "dev_user",
